...
function eventLabel(event: ScouringEvent): string {
  if (event.eventType === 'PARAMETER_ALARM' || event.eventType === 'PARAMETER_WARNING') return 'OUTSIDE EXPECTED RANGE';
  if (event.eventType === 'ALARM_ACKNOWLEDGED') return 'WARNING ACKNOWLEDGED';
  if (event.eventType === 'MACHINE_START' || event.eventType === 'MACHINE_STOP') return 'OPERATOR EVENT';
  return event.eventType.replaceAll('_', ' ');
}

function statusTone(event: ScouringEvent): string {
  if (event.validationStatus === 'alarm') return 'text-warning';
...
export function ScouringAlarmPage() {
  ...
  const activeWarnings = events.filter((event) => (event.eventType === 'PARAMETER_ALARM' || event.eventType === 'PARAMETER_WARNING') && event.acknowledgementStatus === 'UNACKNOWLEDGED');
  ...
  return <main className="h-full min-h-0 overflow-hidden bg-navy text-slate-800 flex flex-col">
    <HMIHeader variant="machine" title="WS3 / Scouring Warning Log" subtitle="Recorded validation and operator events · Manual input source" machineName="SC-01" status="info" time={new Date().toLocaleTimeString('vi-VN')} />
    <div className="mx-2 mt-2 flex min-h-0 flex-1 flex-col overflow-hidden border-2 border-industrialDark bg-hmiConsole">
      <section className="flex-none border-b-2 border-industrialDark">
        <header className="flex min-h-8 items-center justify-between border-b-2 border-industrialDark bg-industrial px-2 py-1 text-white"><h2 className="text-xs font-bold uppercase tracking-wider">Warning / event filters</h2><span className="font-mono text-[10px] uppercase tracking-wider">{filteredEvents.length} / {events.length}</span></header>
        <div className="grid grid-cols-2 gap-1.5 bg-hmiSection p-1.5 sm:grid-cols-3 lg:grid-cols-7">
          ...
          <label className="grid gap-1 text-[10px] font-bold uppercase tracking-wider text-slate-600">Notice status<select className={filterClassName()} value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All</option>{statuses.map((value) => <option key={value}>{value}</option>)}</select></label>
          <HMIButton size="compact" className="self-end" onClick={clearFilters}>Clear filters</HMIButton>
        </div>
      </section>

      <section className={`border-b-2 border-industrialDark ${activeWarnings.length ? "bg-hmiWarning" : "bg-hmiSection"}`}>
        <header className={`flex min-h-8 items-center justify-between px-2 py-1 ${activeWarnings.length ? "border-b-2 border-warning text-warning" : "border-b border-line text-slate-700"}`}><h2 className="text-xs font-bold uppercase tracking-wider">Requires attention</h2><span className="font-mono text-[10px] font-bold">{activeWarnings.length}</span></header>
        <div className="flex flex-wrap gap-2 p-2">{activeWarnings.length === 0 ? <span className="flex items-center gap-2 px-1 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-600"><StatusLamp status="normal" showLabel={false} /> No current warnings</span> : activeWarnings.map((event) => <button key={event.eventId} type="button" className="border-2 border-warning bg-white px-2 py-1 text-left text-xs font-bold text-warning" onClick={() => setSelectedId(event.eventId)}>{event.parameterName ?? event.eventType} · {event.actualValue} {event.unit}</button>)}</div>
      </section>
      ...
      <div className="grid min-h-0 flex-1 gap-2 bg-hmiConsole p-2 lg:grid-cols-[minmax(0,1.08fr)_minmax(380px,0.92fr)]">
        ...
        <section className="flex min-h-0 min-w-0 flex-col bg-hmiInstrument">
          ...
          {selectedEvent ? <div className="min-h-0 flex-1 overflow-auto grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 p-2 text-[11px]"><span className="font-bold uppercase text-slate-500">Event ID</span><span className="font-mono font-bold text-industrial">{selectedEvent.eventId}</span><span className="font-bold uppercase text-slate-500">Recorded at</span><span className="font-mono">{formatDateTime(selectedEvent.timestamp)}</span><span className="font-bold uppercase text-slate-500">Machine / process</span><span>{selectedEvent.machine} · {selectedEvent.process}</span><span className="font-bold uppercase text-slate-500">Batch / operator</span><span>{selectedEvent.batch} · {selectedEvent.operator}</span><span className="font-bold uppercase text-slate-500">Notice</span><span className={`font-mono font-bold ${statusTone(selectedEvent)}`}>{eventLabel(selectedEvent)}</span><span className="font-bold uppercase text-slate-500">Parameter</span><span>{selectedEvent.parameterName ?? '—'}</span><span className="font-bold uppercase text-slate-500">Recorded value</span><span className="font-mono font-bold">{selectedEvent.actualValue !== undefined ? `${selectedEvent.actualValue} ${selectedEvent.unit ?? ''}` : '—'}</span><span className="font-bold uppercase text-slate-500">Reference value</span><span className="font-mono">{selectedEvent.setValue !== undefined ? `${selectedEvent.setValue} ${selectedEvent.unit ?? ''}` : '—'}</span><span className="font-bold uppercase text-slate-500">Expected range</span><span className="font-mono">{selectedEvent.limitRange ?? '—'}</span><span className="font-bold uppercase text-slate-500">Event status</span><span className="font-mono font-bold">{selectedEvent.eventStatus}</span><span className="font-bold uppercase text-slate-500">Warning acknowledgement</span><span className="font-mono font-bold">{selectedEvent.acknowledgementStatus}</span></div> : <div className="p-6 text-center text-xs font-semibold uppercase tracking-wide text-slate-500">Select an event to view its details.</div>}
        </section>
      </div>
    </div>
    <div className="mx-2 mt-2 flex-none"><FunctionKeyBar keys={[{ key: 'F1', label: 'Overview' }, { key: 'F4', label: 'History' }, { key: 'F5', label: 'Warnings', active: true }, { key: 'F6', label: 'Menu' }]} /></div>
  </main>;
}
