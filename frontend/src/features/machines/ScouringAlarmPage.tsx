...
export function ScouringAlarmPage() {
  ...
  return <main className="min-h-screen bg-navy text-slate-800">
    <HMIHeader title="WS3 / Scouring Alarm Log" subtitle="WS3 validation and operator events · Manual input source" machineName="SC-01" status={activeAlarms.length ? 'alarm' : 'info'} time={new Date().toLocaleTimeString('vi-VN')} />
    <div className="mx-2 mt-2 overflow-hidden border-2 border-industrialDark bg-surfaceMuted">
      <section className="border-b-2 border-industrialDark">
        ...
        <div className="grid grid-cols-2 gap-2 bg-[#d3dce0] p-2 sm:grid-cols-3 lg:grid-cols-7">
      ...
      <section className="border-b-2 border-industrialDark bg-[#f8e7e7]">
        <header className="flex min-h-8 items-center justify-between border-b-2 border-alarm px-2 py-1"><h2 className="text-xs font-bold uppercase tracking-wider text-alarm">Active alarms</h2><span className="font-mono text-[10px] font-bold text-alarm">{activeAlarms.length}</span></header>
        <div className="flex flex-wrap gap-2 p-2">{activeAlarms.length === 0 ? <span className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-success"><StatusLamp status="normal" showLabel={false} /> No unacknowledged alarms</span> : activeAlarms.map((event) => <button key={event.eventId} type="button" className="border-2 border-alarm bg-white px-2 py-1 text-left text-xs font-bold text-alarm" onClick={() => setSelectedId(event.eventId)}>{event.parameterName ?? event.eventType} · {event.actualValue} {event.unit}</button>)}</div>
      </section>

      <div className="grid lg:grid-cols-[minmax(0,1.25fr)_minmax(340px,0.75fr)]">
        <section className="min-w-0 border-b-2 border-industrialDark bg-panel lg:border-b-0 lg:border-r-2">
          <header className="flex min-h-8 items-center justify-between border-b-2 border-line bg-surfaceMuted px-2 py-1"><h2 className="text-xs font-bold uppercase tracking-wider">Historical alarm / event records</h2><span className="text-[10px] font-semibold uppercase text-slate-500">Acknowledged events retained</span></header>
          {filteredEvents.length === 0 ? <div className="p-6 text-center text-xs font-semibold uppercase tracking-wide text-slate-500">No events match the filters.</div> : <div className="max-h-[330px] overflow-auto"><table className="w-full min-w-[700px] border-collapse text-left text-xs"><thead className="sticky top-0 bg-industrial text-[10px] uppercase tracking-wider text-white"><tr><th className="px-2 py-2">Date / time</th><th className="px-2 py-2">Event</th><th className="px-2 py-2">Parameter</th><th className="px-2 py-2">Machine</th><th className="px-2 py-2">Operator</th><th className="px-2 py-2">Status</th></tr></thead><tbody>{filteredEvents.map((event) => <tr key={event.eventId} className={`cursor-pointer border-b border-line hover:bg-surfaceMuted ${selectedEvent?.eventId === event.eventId ? 'bg-[#dfeaf0]' : ''}`} onClick={() => setSelectedId(event.eventId)}><td className="whitespace-nowrap px-2 py-2 font-mono">{formatDateTime(event.timestamp)}</td><td className={`px-2 py-2 font-mono text-[10px] font-bold ${statusTone(event)}`}>{eventLabel(event)}</td><td className="px-2 py-2 font-semibold">{event.parameterName ?? '—'}</td><td className="px-2 py-2 font-mono font-bold text-industrial">{event.machine}</td><td className="px-2 py-2">{event.operator}</td><td className={`px-2 py-2 font-mono text-[10px] font-bold ${event.eventStatus === 'ACTIVE' ? 'text-alarm' : event.eventStatus === 'ACKNOWLEDGED' ? 'text-warning' : 'text-slate-500'}`}>{event.eventStatus}</td></tr>)}</tbody></table></div>}
        </section>
        <section className="min-w-0 bg-panel">
          <header className="flex min-h-8 items-center justify-between border-b-2 border-line bg-surfaceMuted px-2 py-1"><h2 className="text-xs font-bold uppercase tracking-wider">Event detail</h2><span className="font-mono text-[10px] text-slate-500">EVENT</span></header>
          {selectedEvent ? <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 p-2 text-[11px]"><span className="font-bold uppercase text-slate-500">Event ID</span><span className="font-mono font-bold text-industrial">{selectedEvent.eventId}</span><span className="font-bold uppercase text-slate-500">Date / time</span><span className="font-mono">{formatDateTime(selectedEvent.timestamp)}</span><span className="font-bold uppercase text-slate-500">Machine / process</span><span>{selectedEvent.machine} · {selectedEvent.process}</span><span className="font-bold uppercase text-slate-500">Batch / operator</span><span>{selectedEvent.batch} · {selectedEvent.operator}</span><span className="font-bold uppercase text-slate-500">Event</span><span className={`font-mono font-bold ${statusTone(selectedEvent)}`}>{eventLabel(selectedEvent)}</span><span className="font-bold uppercase text-slate-500">Parameter</span><span>{selectedEvent.parameterName ?? '—'}</span><span className="font-bold uppercase text-slate-500">Actual</span><span className="font-mono font-bold">{selectedEvent.actualValue !== undefined ? `${selectedEvent.actualValue} ${selectedEvent.unit ?? ''}` : '—'}</span><span className="font-bold uppercase text-slate-500">SET</span><span className="font-mono">{selectedEvent.setValue !== undefined ? `${selectedEvent.setValue} ${selectedEvent.unit ?? ''}` : '—'}</span><span className="font-bold uppercase text-slate-500">Limit / range</span><span className="font-mono">{selectedEvent.limitRange ?? '—'}</span><span className="font-bold uppercase text-slate-500">Event status</span><span className="font-mono font-bold">{selectedEvent.eventStatus}</span><span className="font-bold uppercase text-slate-500">Acknowledgement</span><span className="font-mono font-bold">{selectedEvent.acknowledgementStatus}</span></div> : <div className="p-6 text-center text-xs font-semibold uppercase tracking-wide text-slate-500">Select an event to view its details.</div>}
        </section>
      </div>
    </div>
