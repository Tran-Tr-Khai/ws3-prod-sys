      ...
      <div className="grid min-h-0 flex-1 lg:grid-cols-[278px_minmax(0,1fr)]">
        <aside className="flex flex-col border-b-2 border-industrialDark bg-hmiConsole lg:border-b-0 lg:border-r-2">
          ...
          <section>
            ...
            <div className="flex flex-col gap-2 p-2"><div className={`border-l-4 px-2 py-1 ${alarmCount ? 'border-alarm bg-hmiAlarm' : warningCount ? 'border-warning bg-hmiWarning' : 'border-success bg-hmiNormal'}`}><div className="grid grid-cols-3 divide-x divide-line"><div className="flex items-center gap-1 px-1"><StatusLamp status={alarmCount ? "alarm" : warningCount ? "warning" : "normal"} showLabel={false} /><span className="text-[10px] font-bold uppercase tracking-wider text-slate-600">Alarms</span><span className="ml-auto font-mono text-base font-bold tabular-nums text-slate-800">{alarmCount}</span></div><div className="flex items-center justify-between px-2"><span className="text-[10px] font-bold uppercase tracking-wider text-slate-600">Warning</span><span className="font-mono text-base font-bold tabular-nums text-warning">{warningCount}</span></div><div className="flex items-center justify-between px-2"><span className="text-[10px] font-bold uppercase tracking-wider text-slate-600">Alarm</span><span className="font-mono text-base font-bold tabular-nums text-alarm">{alarmCount}</span></div></div><div className="mt-1 border-t border-line pt-1 text-[10px] font-semibold uppercase tracking-wide text-slate-500">WS3 validation · manual readings</div></div><div className="grid gap-1"><HMIButton size="compact" variant={alarmAcknowledged ? "secondary" : "danger"} className="w-full text-[10px] tracking-wide" disabled={alarmCount === 0 || alarmAcknowledged} onClick={acknowledgeAlarms}>{alarmAcknowledged ? "ACKNOWLEDGED" : "ACKNOWLEDGE ALARM"}</HMIButton><HMIButton size="compact" variant="primary" className="w-full text-[10px] tracking-wide" disabled={alarmCount > 0 && !alarmAcknowledged} onClick={confirmRecord}>{savedRecord ? "RECORD SAVED" : "CONFIRM RECORD"}</HMIButton></div></div>
          </section>
        </aside>
