...
export function HMIHeader({ title, subtitle, machineName, status = 'normal', time = '14:06:40', children, variant = 'default', showStatus = true }: HMIHeaderProps) {
  if (variant === 'machine') {
    return <header className="grid min-h-12 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center border-b border-industrialDark bg-industrial px-2 text-white max-[640px]:min-h-11 max-[640px]:px-1"><div className="min-w-0 pr-1"><div className="truncate text-[13px] font-bold uppercase leading-tight tracking-[0.12em] max-[640px]:text-[11px] max-[640px]:tracking-[0.08em]">{title}</div></div><div className="px-3 text-center text-xs max-[640px]:px-2 max-[640px]:text-[11px]"><span className="font-mono leading-none tabular-nums text-slate-100">{time}</span></div><div className="flex min-w-0 items-center justify-end gap-1 pl-1 text-right text-xs">{children}</div></header>;
  }
