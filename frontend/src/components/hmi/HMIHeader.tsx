...
export function HMIHeader({ title, subtitle, machineName, status = 'normal', time = '14:06:40', children, variant = 'default', showStatus = true }: HMIHeaderProps) {
  if (variant === 'machine') {
    return <header className="grid min-h-12 grid-cols-[minmax(0,1.05fr)_auto_minmax(0,1.45fr)_auto] items-center gap-1 border-b border-industrialDark bg-industrial px-2 text-white"><div className="min-w-0 pr-1"><div className="truncate text-[13px] font-bold uppercase leading-tight tracking-[0.12em]">{title}</div><div className="text-[9px] font-semibold uppercase leading-none tracking-wider text-slate-300">Operator terminal</div></div><div className="flex min-w-0 items-center gap-1 px-1"><span className="font-mono text-xs font-bold leading-tight tracking-wider">{machineName ?? '—'}</span>{showStatus && <span className="[&_*]:text-white"><StatusLamp status={status} label={status === 'running' ? 'RUNNING' : status === 'alarm' ? 'ALARM' : status === 'warning' ? 'WARNING' : status.toUpperCase()} /></span>}</div><div className="min-w-0 truncate px-1 text-[10px] font-semibold uppercase leading-tight tracking-wide text-slate-200">{subtitle ?? '—'}</div><div className="flex min-w-0 items-center justify-end gap-1 pl-1 text-right text-xs"><span className="font-mono leading-none tabular-nums text-slate-100">{time}</span>{children}</div></header>;
  }
